...
SoftwareSerial cell(50,51);  // Create a 'fake' serial port. Pin 2 is the Rx pin, pin 3 is the Tx pin. connect this to the GSM module

const String apn = "gprs.is"; // access-point name for GPRS

const String ip = "89.160.207.212"; // IP address of server we're connecting to
const String host = "Host: sveinnel.com:5000\n"; // required in HTTP 1.1 - what's the name of the host at this IP address?
const String request = "POST /geolog HTTP/1.1\n";                 
String Accept = "Accept: */*\n";
String content = "Content-Type: application/json\n";
const String useragent = "User-Agent: curl/7.35.0\n"; // for our purposes the user agent doesn't matter - if I understand correctly it's helpful to use something generic the server will recognize
String body = "\'{\"date\":\"2014-10-26 10:12\",\"temp\":\"200\"}\'";
String ender ="Content-Length: " + String(body.length()) + "\n\n";
String data = request + host + content + ender + body;   
...
void loop()
{
  ...
  cell.println("AT+SDATACONF=1,\"TCP\",\""+ip+"\",5000");
  waitFor("OK");
  ...
  int packetLength = data.length();//3+host.length()+request.length()+useragent.length()+content.length()+data.length(); // 26 is size of the non-variable parts of the packet, see SIZE comments below
  ...
  // cell.print(request); // SIZE: 2
  // cell.print(useragent); // SIZE: 13
  // cell.print(host); // SIZE: 8
  // // cell.print(Accept); //SIZE 10
  // cell.print(content); //SIZE 16
  // cell.print(String(5)+"\n\n");//SIZE 6
  cell.print(data); //SIZE 2
  ...
  cellHexForever(); // just keep printing whatever GSM module is telling us
}
