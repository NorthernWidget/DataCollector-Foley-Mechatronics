...
readable. The data arrives as a Json array (see figure \ref{JsonArray}), the server parses the data and publishes it if the data is not corrupt. The human readable version is called GeoBlog. The code for the server is quite short and can be seen in the appendix under listings \ref{httpCode}.
